...
The printed summary and `run_list.luminosity_pb(year)` describe the whole list, not the data a production actually processed: they do not follow `--fraction` (which keeps the first input files up to that fraction of the events, i.e. a subset of the runs, not that fraction of the luminosity), a subset of the input files, failed or missing jobs, or `--noRunList` (the runs outside the list have no luminosity in it). stage1 always selects the runs of the default `nominal` policy, so the luminosity of another policy applies only to data restricted to `run_list.good_runs(year, policy)`. For such productions sum `run_list.run_luminosities(year, policy)` (`nominal` by default) over the distinct `run_number` values of the output (exact when whole input files are processed: no 1994 run is split across files). `$ALEPH_RUN_LIST_<year>`, like `$ALEPH_BEAMSPOT_JSON` (the per-run beamspot file of the data), is read by the job itself, and `fccanalysis submit` does not pass the submitting shell's environment to the HTCondor jobs (`getenv = False`): batch jobs use the default files whatever is set at submission.
...
- **`v0_*` is neither exclusive nor single-hypothesis.** It books with the loose windows `AlephLegacyV0::kLoose*`, 0.1–1.4 GeV for both Kₛ and Λ (conversions are never booked), so every booked pair is a Kₛ (`v0_pdg` 310), most pairs are booked again as Λ with the same vertex, and a track can be shared by several entries: `n_v0_ks` counts every accepted pair, and `n_v0_lambda` mostly the same vertices again.
