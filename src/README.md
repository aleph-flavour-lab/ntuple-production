...
[`stage1_Kspipi_example.py`](stage1_Kspipi_example.py) shows how to use the block from a stage1-style script: it runs stage1 and adds a flat `ks_*` block from the tight-Kₛ collection `Ks_event` with the two daughters of every tight Kₛ (charge, momentum at the vertex, dE/dx and, on MC, the PDG code of the linked generator particle via `AlephTruth::trackTruePdg`).
